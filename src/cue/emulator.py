...
from .utils import cont_lam, line_old, logQ, nn_name, nn_wavelength

for this_name in nn_name:
    with open(
        resource_filename("cue", "data/pca_line_new_" + this_name + ".pkl"), "rb"
    ) as f:
        globals()["PCABasis_" + this_name] = pickle.load(f)
    globals()["speculator_" + this_name] = Speculator(
        restore=True,
        restore_filename=resource_filename(
            "cue", "data/speculator_line_new_" + this_name
        ),
    )
...
cont_speculator = Speculator(
    restore=True, restore_filename=resource_filename("cue", "data/speculator_cont_new")
)
...
class Emulator:
    ...
    def __init__(
        self,
        cont_pca_basis=cont_PCABasis,
        cont_nn=cont_speculator,
        line_pca_basis=line_PCABasis,
        line_nn=line_speculator,
        theta=None,
        ionspec_index1=19.7,
        ionspec_index2=5.3,
        ionspec_index3=1.6,
        ...
        **kwargs,
        ...
        if theta is None:
            ...
            if np.size(ionspec_index1) == 1:
                ...
                self.ionspec_index1 = np.reshape(
                    ionspec_index1, (len(ionspec_index1), 1)
                )
                self.ionspec_index2 = np.reshape(
                    ionspec_index2, (len(ionspec_index2), 1)
                )
                self.ionspec_index3 = np.reshape(
                    ionspec_index3, (len(ionspec_index3), 1)
                )
                self.ionspec_index4 = np.reshape(
                    ionspec_index4, (len(ionspec_index4), 1)
                )
                self.ionspec_logLratio1 = np.reshape(
                    ionspec_logLratio1, (len(ionspec_logLratio1), 1)
                )
                self.ionspec_logLratio2 = np.reshape(
                    ionspec_logLratio2, (len(ionspec_logLratio2), 1)
                )
                self.ionspec_logLratio3 = np.reshape(
                    ionspec_logLratio3, (len(ionspec_logLratio3), 1)
                )
            ...
            if np.size(self.ionspec_index1) == 1:
                ...
                self.theta = np.hstack(
                    [
                        ...
                        self.ionspec_index3,
                        self.ionspec_index4,
                        self.ionspec_logLratio1,
                        self.ionspec_logLratio2,
                        self.ionspec_logLratio3,
                        self.gas_logq,
                        10**self.gas_logn,
                        self.gas_logz,
                        self.gas_logno,
                ...
                self.ionspec_index1 = np.reshape(
                    self.ionspec_index1, (len(self.ionspec_index1), 1)
                )
                self.ionspec_index2 = np.reshape(
                    self.ionspec_index2, (len(self.ionspec_index2), 1)
                )
                self.ionspec_index3 = np.reshape(
                    self.ionspec_index3, (len(self.ionspec_index3), 1)
                )
                self.ionspec_index4 = np.reshape(
                    self.ionspec_index4, (len(self.ionspec_index4), 1)
                )
                self.ionspec_logLratio1 = np.reshape(
                    self.ionspec_logLratio1, (len(self.ionspec_logLratio1), 1)
                )
                self.ionspec_logLratio2 = np.reshape(
                    self.ionspec_logLratio2, (len(self.ionspec_logLratio2), 1)
                )
                self.ionspec_logLratio3 = np.reshape(
                    self.ionspec_logLratio3, (len(self.ionspec_logLratio3), 1)
                )
    ...
    def predict_cont(self, wave, **kwargs):
        ...
        if self.cont_n_segments == 1:
            fit_spectra = (
                self.cont_pca_basis.PCA.inverse_transform(
                    self.cont_nn.log_spectrum_(self.theta)
                )
                * self.cont_nn.log_spectrum_scale_
                + self.cont_nn.log_spectrum_shift_
            )
            if self.n_sample == 1:
            ...
            for j in range(self.cont_n_segments):
                this_spec.append(
                    self.cont_pca_basis[j].PCA.inverse_transform(
                        self.cont_nn[j].log_spectrum_(self.theta)
                    )
                    * self.cont_nn[j].log_spectrum_scale_
                    + self.cont_nn[j].log_spectrum_shift_
                )
        ...
        self.cont_nn_spectra = 10 ** (
            self.cont_nn_spectra - self.gas_logq + self.log_qion - np.log10(3.839e33)
        )
        self.output_cont_wavelength = self.cont_wavelength[wavind_sorted]
        from scipy.interpolate import CubicSpline

        neb_cont_cs = CubicSpline(
            self.cont_wavelength, self.cont_nn_spectra, extrapolate=True
        )  # interpolate onto the fsps wavelengths
    ...
    def predict_lines(self, **kwargs):
        ...
        if self.line_n_segments == 1:
            fit_spectra = (
                self.line_pca_basis.PCA.inverse_transform(
                    self.line_nn.log_spectrum_(self.theta)
                )
                * self.line_nn.log_spectrum_scale_
                + self.line_nn.log_spectrum_shift_
            )
            if self.n_sample == 1:
                fit_spectra = np.squeeze(fit_spectra)[wavind_sorted]
            else:
            ...
            for j in range(self.line_n_segments):
                this_spec.append(
                    self.line_pca_basis[j].PCA.inverse_transform(
                        self.line_nn[j].log_spectrum_(self.theta)
                    )
                    * self.line_nn[j].log_spectrum_scale_
                    + self.line_nn[j].log_spectrum_shift_
                )
        ...
        self.line_nn_spectra = 10 ** (
            self.line_nn_spectra - self.gas_logq + self.log_qion - np.log10(3.839e33)
        )
